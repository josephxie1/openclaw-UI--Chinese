...
export async function loadPluginSchema(
  ...
  // Already loaded or loading — guard against undefined (Zustand state may lack these fields)
  if (state.pluginSchemaCache?.[pluginId] || state.pluginSchemaLoading?.has(pluginId)) {
    return;
  }
  if (!state.pluginSchemaLoading) state.pluginSchemaLoading = new Set();
  ...
  try {
    ...
    if (res?.pluginSchema?.entry) {
      if (!state.pluginSchemaCache) state.pluginSchemaCache = {};
