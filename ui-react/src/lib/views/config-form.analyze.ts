...
function normalizeUnion(
  ...
  // Literales + strings restantes (ej: domain = const "feishu" | const "lark" | string con pattern)
  // Colapsar a enum + campo de texto libre
  if (
    literals.length > 0 &&
    remaining.length > 0 &&
    remaining.every((entry) => schemaType(entry) === "string")
  ) {
    const unique: unknown[] = [];
    for (const value of literals) {
      if (!unique.some((existing) => Object.is(existing, value))) {
  ...
  // string | array<T> (ej: allowFrom = string | string[])
  // Normalizar como array para la UI
  if (remaining.length === 2 && literals.length === 0) {
    const stringEntry = remaining.find((e) => schemaType(e) === "string");
    const arrayEntry = remaining.find((e) => schemaType(e) === "array");
    if (stringEntry && arrayEntry) {
      const itemsSchema = Array.isArray(arrayEntry.items) ? arrayEntry.items[0] : arrayEntry.items;
      const res = normalizeSchemaNode(
        { ...arrayEntry, items: itemsSchema ?? { type: "string" }, nullable },
        path,
      );
      return res;
    }
  ...
  // Degradación elegante: pasar el anyOf/oneOf intacto al renderer,
  // que lo mostrará como textarea JSON en lugar de "no soportado"
  const unsupported: string[] = [];
  for (const entry of remaining) {
    if (schemaType(entry) === "object" && entry.properties) {
