    ...
    <div class="active-filters">
        ...
        selectedDays.length > 0
          ? html`
            <div class="filter-chip">
              <span class="filter-chip-label">${t("usageView.daysLabel")} ${daysLabel}</span>
      ...
      ${
        selectedHours.length > 0
          ? html`
            <div class="filter-chip">
              <span class="filter-chip-label">${t("usageView.hoursLabel")} ${hoursLabel}</span>
        ...
        selectedSessions.length > 0
          ? html`
            <div class="filter-chip" title="${sessionsFullName}">
              <span class="filter-chip-label">${t("usageView.sessionLabel")} ${sessionsLabel}</span>
  ...
  if (!daily.length) {
    return html`
      <div class="daily-chart-compact">
        <div class="sessions-panel-title">${t("usageView.dailyUsage")}</div>
        <div class="muted" style="padding: 20px; text-align: center">${t("usageView.noData")}</div>
      </div>
    `;
  }
  ...
  return html`
    <div class="daily-chart-compact">
      <div class="daily-chart-header">
        ...
        <div class="card-title">${isTokenMode ? t("usageView.dailyTokenUsage") : t("usageView.dailyCostUsage")}</div>
      </div>
          ...
          ${daily.map((d, idx) => {
            ...
            const breakdownLines =
              dailyChartMode === "by-type"
                ? isTokenMode
                  ? [
                      `${t("usageView.output")} ${formatTokens(d.output)}`,
                      `${t("usageView.input")} ${formatTokens(d.input)}`,
                      `${t("usageView.cacheWrite")} ${formatTokens(d.cacheWrite)}`,
                      `${t("usageView.cacheRead")} ${formatTokens(d.cacheRead)}`,
                    ]
                  : [
                      `${t("usageView.output")} ${formatCost(d.outputCost ?? 0)}`,
                      `${t("usageView.input")} ${formatCost(d.inputCost ?? 0)}`,
                      `${t("usageView.cacheWrite")} ${formatCost(d.cacheWriteCost ?? 0)}`,
                      `${t("usageView.cacheRead")} ${formatCost(d.cacheReadCost ?? 0)}`,
                    ]
                : [];
...
function renderCostBreakdownCompact(totals: UsageTotals, mode: "tokens" | "cost") {
  ...
  return html`
    <div class="cost-breakdown cost-breakdown-compact">
      <div class="cost-breakdown-header">${isTokenMode ? t("usageView.tokensByType") : t("usageView.costByType")}</div>
      <div class="cost-breakdown-bar">
        <div class="cost-segment output" style="width: ${(isTokenMode ? tokenPcts.output : breakdown.output.pct).toFixed(1)}%"
          title="${t("usageView.output")}: ${isTokenMode ? formatTokens(totals.output) : formatCost(breakdown.output.cost)}"></div>
        <div class="cost-segment input" style="width: ${(isTokenMode ? tokenPcts.input : breakdown.input.pct).toFixed(1)}%"
          title="${t("usageView.input")}: ${isTokenMode ? formatTokens(totals.input) : formatCost(breakdown.input.cost)}"></div>
        <div class="cost-segment cache-write" style="width: ${(isTokenMode ? tokenPcts.cacheWrite : breakdown.cacheWrite.pct).toFixed(1)}%"
          title="${t("usageView.cacheWrite")}: ${isTokenMode ? formatTokens(totals.cacheWrite) : formatCost(breakdown.cacheWrite.cost)}"></div>
        <div class="cost-segment cache-read" style="width: ${(isTokenMode ? tokenPcts.cacheRead : breakdown.cacheRead.pct).toFixed(1)}%"
          title="${t("usageView.cacheRead")}: ${isTokenMode ? formatTokens(totals.cacheRead) : formatCost(breakdown.cacheRead.cost)}"></div>
      </div>
      <div class="cost-breakdown-legend">
        <span class="legend-item"><span class="legend-dot output"></span>${t("usageView.output")} ${isTokenMode ? formatTokens(totals.output) : formatCost(breakdown.output.cost)}</span>
        <span class="legend-item"><span class="legend-dot input"></span>${t("usageView.input")} ${isTokenMode ? formatTokens(totals.input) : formatCost(breakdown.input.cost)}</span>
        <span class="legend-item"><span class="legend-dot cache-write"></span>${t("usageView.cacheWrite")} ${isTokenMode ? formatTokens(totals.cacheWrite) : formatCost(breakdown.cacheWrite.cost)}</span>
        <span class="legend-item"><span class="legend-dot cache-read"></span>${t("usageView.cacheRead")} ${isTokenMode ? formatTokens(totals.cacheRead) : formatCost(breakdown.cacheRead.cost)}</span>
      </div>
...
function renderUsageInsights(
  ...
  const avgDurationLabel =
    stats.durationCount > 0
      ...
      : "—";
  const cacheHint = t("usageView.cacheHitHint");
  const errorHint = t("usageView.errorRateHint");
  const throughputHint = t("usageView.throughputHint");
  const tokensHint = t("usageView.tokensHint");
  const costHint = showCostHint
    ? t("usageView.avgCostMissing")
    : t("usageView.avgCostNormal");
  ...
  return html`
    <section class="card" style="margin-top: 16px;">
      <div class="card-title">${t("usageView.usageOverview")}</div>
      <div class="usage-summary-grid">
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t("usageView.messages")}
            <span class="usage-summary-hint" title=${t("usageView.messagesHint")}>?</span>
          </div>
          <div class="usage-summary-value">${aggregates.messages.total}</div>
          <div class="usage-summary-sub">
            ${aggregates.messages.user} ${t("usageView.userLabel")} · ${aggregates.messages.assistant} ${t("usageView.assistantLabel")}
          </div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t("usageView.toolCalls")}
            <span class="usage-summary-hint" title=${t("usageView.toolCallsHint")}>?</span>
          </div>
          <div class="usage-summary-value">${aggregates.tools.totalCalls}</div>
          <div class="usage-summary-sub">${aggregates.tools.uniqueTools} ${t("usageView.toolsUsed")}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t("usageView.errors")}
            <span class="usage-summary-hint" title=${t("usageView.errorsHint")}>?</span>
          </div>
          <div class="usage-summary-value">${aggregates.messages.errors}</div>
          <div class="usage-summary-sub">${aggregates.messages.toolResults} ${t("usageView.toolResults")}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t("usageView.avgTokensMsg")}
            <span class="usage-summary-hint" title=${tokensHint}>?</span>
          </div>
          <div class="usage-summary-value">${formatTokens(avgTokens)}</div>
          <div class="usage-summary-sub">${t("usageView.acrossMessages", { count: String(aggregates.messages.total || 0) })}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t("usageView.avgCostMsg")}
            <span class="usage-summary-hint" title=${costHint}>?</span>
          </div>
          <div class="usage-summary-value">${formatCost(avgCost, 4)}</div>
          <div class="usage-summary-sub">${formatCost(totals.totalCost)} ${t("usageView.totalLabel")}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t("usageView.sessionsLabel")}
            <span class="usage-summary-hint" title=${t("usageView.sessionsHint")}>?</span>
          </div>
          <div class="usage-summary-value">${sessionCount}</div>
          <div class="usage-summary-sub">${t("usageView.ofInRange", { total: String(totalSessions) })}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t("usageView.throughput")}
        ...
        <div class="usage-summary-card">
          ...
          <div class="usage-summary-sub">
            ${t("usageView.errorsAvgSession", { errors: String(aggregates.messages.errors), duration: avgDurationLabel })}
          </div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t("usageView.cacheHitRate")}
          ...
          <div class="usage-summary-sub">
            ${t("usageView.cachedPrompt", { cached: formatTokens(totals.cacheRead), prompt: formatTokens(cacheBase) })}
          </div>
        </div>
      </div>
      <div class="usage-insights-grid">
        ${renderInsightList(t("usageView.topModels"), topModels, t("usageView.noModelData"))}
        ${renderInsightList(t("usageView.topProviders"), topProviders, t("usageView.noProviderData"))}
        ${renderInsightList(t("usageView.topTools"), topTools, t("usageView.noToolCalls"))}
        ${renderInsightList(t("usageView.topAgents"), topAgents, t("usageView.noAgentData"))}
        ${renderInsightList(t("usageView.topChannels"), topChannels, t("usageView.noChannelData"))}
        ${renderPeakErrorList(t("usageView.peakErrorDays"), errorDays, t("usageView.noErrorData"))}
        ${renderPeakErrorList(t("usageView.peakErrorHours"), errorHours, t("usageView.noErrorData"))}
      </div>
    </section>
  `;
}
      ...
      >
        ...
        <div class="session-bar-actions">
          <button
            ...
            title=${t("usageView.copySessionName")}
            @click=${(e: MouseEvent) => {
              e.stopPropagation();
  ...
  return html`
    <div class="card sessions-card">
      <div class="sessions-card-header">
        <div class="card-title">${t("usageView.sessionsLabel")}</div>
      ...
      <div class="sessions-card-meta">
        ...
        <label class="sessions-sort">
          ...
          >
            <option value="cost" ?selected=${sessionSort === "cost"}>${t("usageView.costSort")}</option>
            <option value="errors" ?selected=${sessionSort === "errors"}>${t("usageView.errorsSort")}</option>
            <option value="messages" ?selected=${sessionSort === "messages"}>${t("usageView.messagesSort")}</option>
            <option value="recent" ?selected=${sessionSort === "recent"}>${t("usageView.recentSort")}</option>
            <option value="tokens" ?selected=${sessionSort === "tokens"}>${t("usageView.tokensSort")}</option>
          </select>
        </label>
        <button
      ...
      ${
        sessionsTab === "recent"
          ? recentEntries.length === 0
            ? html`
                <div class="muted" style="padding: 20px; text-align: center">${t("usageView.noRecentSessions")}</div>
              `
            : html`
          ...
          : sessions.length === 0
            ? html`
                <div class="muted" style="padding: 20px; text-align: center">${t("usageView.noSessionsInRange")}</div>
              `
            : html`
	                <div class="session-bars">
	                  ...
	                  ${sessions.length > 50 ? html`<div class="muted" style="padding: 8px; text-align: center; font-size: 11px;">${t("usageView.more", { count: String(sessions.length - 50) })}</div>` : nothing}
	                </div>
	              `
      }
      ${
        selectedCount > 1
          ? html`
              <div style="margin-top: 10px;">
                <div class="sessions-card-count">${t("usageView.selectedCount", { count: String(selectedCount) })}</div>
