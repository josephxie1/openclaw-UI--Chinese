...
import { d as hasNodeErrorCode, f as isNotFoundPathError, m as isSymlinkOpenError, p as isPathInside, s as sameFileIdentity } from "./openclaw-root-CMyEcfUW.js";
...
async function writeFileWithinRoot(params) {
	...
	try {
		try {
			handle = await fs$1.open(ioPath, OPEN_WRITE_EXISTING_FLAGS, 384);
		} catch (err) {
			if (!isNotFoundPathError(err)) throw err;
			handle = await fs$1.open(ioPath, OPEN_WRITE_CREATE_FLAGS, 384);
	...
	try {
		...
		if (createdForWrite && err instanceof SafeOpenError && openedRealPath) await fs$1.rm(openedRealPath, { force: true }).catch(() => {});
		throw err;
	} finally {
		await handle.close().catch(() => {});
	}
