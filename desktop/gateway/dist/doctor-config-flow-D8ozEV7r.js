import { f as resolveLegacyStateDirs, g as resolveStateDir, m as resolveOAuthDir, p as resolveNewStateDir, t as CONFIG_PATH } from "./paths-DPETYfXn.js";
import { E as normalizeChatChannelId } from "./subsystem-Cg_UWVEG.js";
import { g as resolveHomeDir, u as isRecord } from "./utils-CfiuWRBq.js";
import { Dn as resolveTelegramPreviewStreamMode, En as resolveSlackStreamingMode, Gt as isTrustedSafeBinPath, O as migrateLegacyConfig, P as readConfigFileSnapshot, Tn as resolveSlackNativeStreaming, U as OpenClawSchema, Wt as getTrustedSafeBinDirs, in as resolveCommandResolutionFromArgv, qt as normalizeTrustedSafeBinDirs, wn as resolveDiscordPreviewStreamMode } from "./auth-profiles-pmKds52w.js";
import { t as formatCliCommand } from "./command-format-D-E6F1gN.js";
import { d as resolveDefaultAgentId } from "./agent-scope-CLHs7Zj4.js";
import { h as normalizeAccountId, m as DEFAULT_ACCOUNT_ID, n as DEFAULT_MAIN_KEY, r as buildAgentMainSessionKey, s as normalizeAgentId } from "./session-key-Bbgq13tJ.js";
import { f as parseToolsBySenderTypedKey } from "./dock-CMVWLgFK.js";
import { ft as canonicalizeMainSessionAlias, g as saveSessionStore } from "./sessions-BDU_-DdW.js";
import { c as resolveTelegramAccount, o as listTelegramAccountIds } from "./plugins-CD0OuhLK.js";
...
import { t as applyPluginAutoEnable } from "./plugin-auto-enable-CMAS0I3y.js";
import { t as collectProviderDangerousNameMatchingScopes } from "./dangerous-name-matching-CSq9sJDG.js";
import { r as shouldMoveSingleAccountChannelKey, t as fetchTelegramChatId } from "./api-KwdCa2lm.js";
import { t as note } from "./note-CztWsdZ6.js";
import { t as isWithinDir } from "./path-safety-DZLytQxg.js";
import { a as isMattermostMutableAllowEntry, c as normalizeTelegramAllowFromEntry, i as isMSTeamsMutableAllowEntry, n as isGoogleChatMutableAllowEntry, o as isSlackMutableAllowEntry, r as isIrcMutableAllowEntry, s as isNumericTelegramUserId, t as isDiscordMutableAllowEntry } from "./mutable-allowlist-detectors-COMtneie.js";
...
function collectMissingDefaultAccountBindingWarnings(cfg) {
	...
	for (const [channelKey, rawChannel] of Object.entries(channels)) {
		...
		const normalizedAccountIds = Array.from(new Set(Object.keys(accounts).map((accountId) => normalizeAccountId(accountId)).filter(Boolean)));
		...
		if (coveredAccountIds.size > 0) {
			warnings.push(`- channels.${channelKey}: accounts.default is missing and account bindings only cover a subset of configured accounts. Uncovered accounts: ${uncoveredAccountIds.join(", ")}. Add bindings[].match.accountId for uncovered accounts (or "*"), or add channels.${channelKey}.accounts.default.`);
			continue;
		}
		warnings.push(`- channels.${channelKey}: accounts.default is missing and no valid account-scoped binding exists for configured accounts (${normalizedAccountIds.join(", ")}). Channel-only bindings (no accountId) match only default. Add bindings[].match.accountId for one of these accounts (or "*"), or add channels.${channelKey}.accounts.default.`);
	}
	return warnings;
}
