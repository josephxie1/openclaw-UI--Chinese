import { f as isNotFoundPathError, m as isSymlinkOpenError, p as isPathInside } from "./openclaw-root-3Za7U6qY.js";
import { n as resolveSafeBaseDir } from "./path-safety-C_fs8VTy.js";
...
async function openZipOutputFile(outPath, originalPath) {
	try {
		return await fsPromises.open(outPath, OPEN_WRITE_FLAGS, 438);
	} catch (err) {
...
async function writeZipFileEntry(params) {
	...
	if (typeof params.entry.unixPermissions === "number") {
		...
		if (mode !== 0) await fsPromises.chmod(params.outPath, mode).catch(() => void 0);
	}
}
