...
import { n as markPwAiLoaded } from "./diagnostic-session-state-An6nqjMW.js";
import { _ as appendCdpPath, b as getHeadersWithAuth, d as normalizeCdpWsUrl, g as withBrowserNavigationPolicy, h as assertBrowserNavigationResultAllowed, m as assertBrowserNavigationAllowed, t as getChromeWebSocketUrl, u as formatAriaSnapshot, v as fetchJson, w as withNoProxyForLocalhost, x as withCdpSocket } from "./chrome-vkPv6Hbo.js";
import { a as resolveStrictExistingPathsWithinRoot, r as DEFAULT_UPLOAD_DIR, s as DEFAULT_FILL_FIELD_TYPE } from "./paths-CjKUISLf.js";
...
async function connectBrowser(cdpUrl) {
	...
	const connectWithRetry = async () => {
		...
		for (let attempt = 0; attempt < 3; attempt += 1) try {
			...
			const browser = await withNoProxyForLocalhost(() => chromium.connectOverCDP(endpoint, {
				timeout,
				headers
			}));
...
async function writeViaSiblingTempPath(params) {
	...
	try {
		...
		if (!renameSucceeded) await fsPromises.rm(tempPath, { force: true }).catch(() => {});
	}
}
...
async function saveDownloadPayload(download, outPath) {
	...
	await fsPromises.mkdir(path.dirname(resolvedOutPath), { recursive: true });
