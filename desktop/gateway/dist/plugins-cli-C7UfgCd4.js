...
import { b as shortenHomeInString, h as resolveConfigDir, v as resolveUserPath, x as shortenHomePath, z as theme } from "./utils-CfiuWRBq.js";
import "./boolean-YY6K2DFz.js";
import { M as loadConfig, R as writeConfigFile } from "./auth-profiles-pmKds52w.js";
...
import { m as defaultSlotIdForKey, p as applyExclusiveSlotSelection, s as resolveBundledPluginsDir, t as clearPluginManifestRegistryCache } from "./manifest-registry-NoROo1B_.js";
...
import { l as promptYesNo } from "./tailscale-DYJAYqJb.js";
...
import { n as setPluginEnabledInConfig, t as enablePluginInConfig } from "./enable-DeTlg5HR.js";
import { c as resolveArchiveKind } from "./install-safe-path-DgtNlgzD.js";
import "./npm-pack-install-BEFHzYLl.js";
import "./skill-scanner-DYZcWH6U.js";
import { a as resolvePluginInstallDir, i as installPluginFromPath, n as recordPluginInstall, r as installPluginFromNpmSpec } from "./installs-7NF1DffQ.js";
import { t as renderTable } from "./table-D_8bv6eo.js";
import { n as resolvePinnedNpmInstallRecordForCli, r as buildPluginStatusReport } from "./npm-resolution-D74KQVes.js";
import { n as updateNpmInstalledPlugins, r as findBundledPluginByNpmSpec } from "./update-CybsAOYO.js";
...
function isPackageNotFoundInstallError(message) {
	const lower = message.toLowerCase();
	return lower.includes("npm pack failed:") && (lower.includes("e404") || lower.includes("404 not found") || lower.includes("could not be found"));
}
function registerPluginsCli(program) {
	...
	plugins.command("install").description("Install a plugin (path, archive, or npm spec)").argument("<path-or-spec>", "Path (.ts/.js/.zip/.tgz/.tar.gz) or an npm package spec").option("-l, --link", "Link a local path instead of copying", false).option("--pin", "Record npm installs as exact resolved <name>@<version>", false).action(async (raw, opts) => {
		...
		if (!result.ok) {
			const bundledFallback = isPackageNotFoundInstallError(result.error) ? findBundledPluginByNpmSpec({ spec: raw }) : void 0;
			if (!bundledFallback) {
				defaultRuntime.error(result.error);
				process.exit(1);
			}
				...
				plugins: {
					...
					entries: {
						...
						[bundledFallback.pluginId]: {
							...cfg.plugins?.entries?.[bundledFallback.pluginId],
							enabled: true
						}
					}
				}
			...
			await writeConfigFile(next);
			logSlotWarnings(slotResult.warnings);
			defaultRuntime.log(theme.warn(`npm package unavailable for ${raw}; using bundled plugin at ${shortenHomePath(bundledFallback.localPath)}.`));
