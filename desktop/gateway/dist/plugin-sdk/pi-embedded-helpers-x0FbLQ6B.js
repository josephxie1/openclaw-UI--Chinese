import { l as resolveAgentIdFromSessionKey, s as normalizeAgentId } from "./session-key-47BV2tLd.js";
import { a as resolveGatewayPort, n as STATE_DIR } from "./paths-DCNrSyZW.js";
import { B as getBlockedNetworkModeReason, n as loadConfig, o as writeConfigFile, t as createConfigIO } from "./config-Zs384y2C.js";
import { H as resolveUserPath, Y as truncateUtf16Safe, c as CHANNEL_IDS, i as defaultRuntime, t as createSubsystemLogger } from "./subsystem-D7KkLxSJ.js";
import { i as parseBooleanValue, t as formatCliCommand } from "./command-format-D4smYdZ1.js";
import { D as runExec, P as openBoundaryFile, _ as DEFAULT_IDENTITY_FILENAME, b as DEFAULT_USER_FILENAME, d as resolveSessionAgentId, g as DEFAULT_HEARTBEAT_FILENAME, h as DEFAULT_BOOTSTRAP_FILENAME, m as DEFAULT_AGENT_WORKSPACE_DIR, p as DEFAULT_AGENTS_FILENAME, r as resolveAgentConfig, v as DEFAULT_SOUL_FILENAME, x as ensureAgentWorkspace, y as DEFAULT_TOOLS_FILENAME, z as resolvePathViaExistingAncestorSync } from "./agent-scope-DmMs4gD_.js";
import { U as canonicalizeMainSessionAlias, W as resolveAgentMainSessionKey, j as acquireSessionWriteLock } from "./sessions-Hs8-QPv2.js";
import { n as formatErrorMessage, t as extractErrorCode } from "./errors-BMOVwRE7.js";
import { t as PATH_ALIAS_POLICIES } from "./path-alias-guards-mdyCpWU6.js";
import { i as getImageMetadata, n as buildImageResizeSideGrid, s as resizeToJpeg, t as IMAGE_REDUCE_QUALITY_STEPS } from "./image-ops-CCi2A8ff.js";
import { t as SsrFBlockedError } from "./ssrf-D-Eyl7pQ.js";
import { t as sanitizeContentBlocksImages } from "./tool-images-BcBgoCop.js";
import { i as resolveWindowsSpawnProgram, n as materializeWindowsSpawnProgram } from "./windows-spawn-BTbbRc56.js";
import { a as syncSkillsToWorkspace, h as safeEqualSecret, l as resolveSandboxInputPath, m as sanitizeEnvVars, u as resolveSandboxPath } from "./skills-B6Tk93F8.js";
import { B as DEFAULT_OPENCLAW_BROWSER_ENABLED, C as ensureChromeExtensionRelayServer, D as DEFAULT_TRACE_DIR, E as DEFAULT_DOWNLOAD_DIR, F as DEFAULT_AI_SNAPSHOT_EFFICIENT_MAX_CHARS, H as isLoopbackHost, I as DEFAULT_AI_SNAPSHOT_MAX_CHARS, L as DEFAULT_BROWSER_DEFAULT_PROFILE_NAME, N as normalizeBrowserFormField, O as DEFAULT_UPLOAD_DIR, P as DEFAULT_AI_SNAPSHOT_EFFICIENT_DEPTH, R as DEFAULT_BROWSER_EVALUATE_ENABLED, V as DEFAULT_OPENCLAW_BROWSER_PROFILE_NAME, _ as withBrowserNavigationPolicy, a as resolveOpenClawUserDataDir, b as fetchOk, c as resolveBrowserExecutableForPlatform, f as normalizeCdpWsUrl, g as assertBrowserNavigationResultAllowed, h as assertBrowserNavigationAllowed, i as launchOpenClawChrome, j as resolveWritablePathWithinRoot, k as resolveExistingPathsWithinRoot, l as captureScreenshot, m as InvalidBrowserNavigationUrlError, n as isChromeCdpReady, o as stopOpenClawChrome, p as snapshotAria, r as isChromeReachable, u as createTargetViaCdp, v as appendCdpPath, w as stopChromeExtensionRelayServer, y as fetchJson, z as DEFAULT_OPENCLAW_BROWSER_COLOR } from "./chrome-CsOXUZkg.js";
import { i as saveMediaBuffer, t as ensureMediaDir } from "./store--pUWkusa.js";
import { o as normalizeThinkLevel } from "./thinking-BgIZOmA_.js";
import { t as generateSecureToken } from "./secure-random-CAr24RRq.js";
...
async function loadPwAiModule(mode) {
	try {
		return await import("./pw-ai-5V8_sm4A.js");
	} catch (err) {
...
function resolveBrowserConfig(cfg, rootConfig) {
	...
	const defaultProfile = defaultProfileFromConfig ?? ((headless || noSandbox) && profiles[DEFAULT_OPENCLAW_BROWSER_PROFILE_NAME] ? DEFAULT_OPENCLAW_BROWSER_PROFILE_NAME : profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME] ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME : DEFAULT_OPENCLAW_BROWSER_PROFILE_NAME);
...
function createProfileContext(opts, profile) {
	...
	const enforceManagedTabLimit = async (keepTargetId) => {
		...
		if (pageTabs.length <= MAX_MANAGED_BROWSER_PAGE_TABS) return;
	...
	const ensureBrowserAvailable = async () => {
		...
		if (isExtension) {
			if (!httpReachable) {
				...
				if (!await isHttpReachable(1200)) throw new Error(`Chrome extension relay for profile "${profile.name}" is not reachable at ${profile.cdpUrl}.`);
			}
			return;
		}
		if (!httpReachable) {
		...
		if (!profileState.running) throw new Error(`Unexpected state for profile "${profile.name}": no running process to restart.`);
	...
	const resetProfile = async () => {
		...
		if (await isHttpReachable(300) && !profileState.running) try {
			await (await import("./pw-ai-5V8_sm4A.js")).closePlaywrightBrowserConnection();
		} catch {}
...
var SandboxFsBridgeImpl = class {
	...
	async assertPathSafety(target, options) {
		...
		if (!guarded.ok) {
			if (guarded.reason !== "path") throw guarded.error instanceof Error ? guarded.error : /* @__PURE__ */ new Error(`Sandbox boundary checks failed; cannot ${options.action}: ${target.containerPath}`);
...
//#endregion
export { expandToolGroups as $, isRawApiErrorPayload as A, registerBrowserRoutes as B, isCloudCodeAssistFormatError as C, isFailoverErrorMessage as D, isFailoverAssistantError as E, sanitizeUserFacingText as F, resolveBrowserControlAuth as G, resolveProfile as H, ensureSandboxWorkspaceForSession as I, buildPluginToolGroups as J, resolveSandboxConfigForAgent as K, resolveSandboxContext as L, isTransientHttpError as M, parseImageDimensionError as N, isLikelyContextOverflowError as O, parseImageSizeError as P, stripPluginOnlyAllowlist as Q, resolveSandboxRuntimeStatus as R, isBillingAssistantError as S, isContextOverflowError as T, getBridgeAuthForPort as U, resolveBrowserConfig as V, ensureBrowserControlAuth as W, expandPolicyWithPluginGroups as X, collectExplicitAllowlist as Y, mergeAlsoAllowPolicy as Z, formatBillingErrorMessage as _, isMessagingToolDuplicateNormalized as a, ensureSessionHeader as at, isAuthAssistantError as b, extractToolCallsFromAssistant as c, sanitizeGoogleTurnOrdering as ct, downgradeOpenAIFunctionCallReasoningPairs as d, normalizeToolName as et, downgradeOpenAIReasoningBlocks as f, formatAssistantErrorText as g, classifyFailoverReason as h, isMessagingToolDuplicate as i, buildBootstrapContextFiles as it, isTimeoutErrorMessage as j, isRateLimitAssistantError as k, extractToolResultId as l, resolveGatewayCredentialsFromConfig as lt, BILLING_ERROR_USER_MESSAGE as m, validateGeminiTurns as n, compileGlobPatterns as nt, normalizeTextForComparison as o, resolveBootstrapMaxChars as ot, isGoogleModelApi as p, applyOwnerOnlyToolPolicy as q, pickFallbackThinkingLevel as r, matchesAnyGlobPattern as rt, sanitizeSessionMessagesImages as s, resolveBootstrapTotalMaxChars as st, validateAnthropicTurns as t, resolveToolProfilePolicy as tt, sanitizeToolCallIdsForCloudCodeAssist as u, trimToUndefined as ut, formatRawAssistantErrorForUi as v, isCompactionFailureError as w, isAuthPermanentErrorMessage as x, getApiErrorPayloadFingerprint as y, createBrowserRouteContext as z };