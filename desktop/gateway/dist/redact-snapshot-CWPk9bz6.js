...
const FIELD_HELP = {
	...
	"acp.dispatch.enabled": "Independent dispatch gate for ACP session turns. Disable to keep ACP commands available while blocking ACP turn execution.",
	...
	"diagnostics.stuckSessionWarnMs": "Age threshold in milliseconds for emitting stuck-session warnings while a session remains in processing state. Increase for long multi-tool turns to reduce false positives; decrease for faster hang detection.",
	...
	"tools.web.search.provider": "Search provider (\"brave\", \"perplexity\", \"grok\", \"gemini\", or \"kimi\"). Auto-detected from available API keys if omitted.",
	...
	"agents.defaults.embeddedPi": "Embedded Pi runner hardening controls for how workspace-local Pi settings are trusted and applied in OpenClaw sessions.",
	"agents.defaults.embeddedPi.projectSettingsPolicy": "How embedded Pi handles workspace-local `.pi/config/settings.json`: \"sanitize\" (default) strips shellPath/shellCommandPrefix, \"ignore\" disables project settings entirely, and \"trusted\" applies project settings as-is.",
	...
	"session.parentForkMaxTokens": "Maximum parent-session token count allowed for thread/session inheritance forking. If the parent exceeds this, OpenClaw starts a fresh thread session instead of forking; set 0 to disable this protection.",
	...
	"cron.retry": "Overrides the default retry policy for one-shot jobs when they fail with transient errors (rate limit, network, server_error). Omit to use defaults: maxAttempts 3, backoffMs [30000, 60000, 300000], retry all transient types.",
	...
	"cron.retry.retryOn": "Error types to retry: rate_limit, network, timeout, server_error. Use to restrict which errors trigger retries; omit to retry all transient types.",
	...
	"channels.discord.eventQueue.listenerTimeout": "Canonical Discord listener timeout control in ms for gateway event handlers. Default is 120000 in OpenClaw; set per account via channels.discord.accounts.<id>.eventQueue.listenerTimeout.",
	"channels.discord.eventQueue.maxQueueSize": "Optional Discord EventQueue capacity override (max queued events before backpressure). Set per account via channels.discord.accounts.<id>.eventQueue.maxQueueSize.",
	"channels.discord.eventQueue.maxConcurrency": "Optional Discord EventQueue concurrency override (max concurrent handler executions). Set per account via channels.discord.accounts.<id>.eventQueue.maxConcurrency.",
	...
	"channels.discord.threadBindings.idleHours": "Inactivity window in hours for Discord thread-bound sessions (/focus and spawned thread sessions). Set 0 to disable idle auto-unfocus (default: 24). Overrides session.threadBindings.idleHours when set.",
	"channels.discord.threadBindings.maxAgeHours": "Optional hard max age in hours for Discord thread-bound sessions. Set 0 to disable hard cap (default: 0). Overrides session.threadBindings.maxAgeHours when set.",
	...
	"channels.discord.threadBindings.spawnAcpSessions": "Allow /acp spawn to auto-create and bind Discord threads for ACP sessions (default: false; opt-in). Set true to enable thread-bound ACP spawns for this account/channel.",
...
function collectSensitiveStrings(value, values) {
	...
	if (value && typeof value === "object") for (const item of Object.values(value)) collectSensitiveStrings(item, values);
}
...
function redactConfigSnapshot(snapshot, uiHints, options) {
...
*/
function redactConfigRaw(snapshot, uiHints) {
	if (!snapshot.valid || !snapshot.raw) return {
		raw: null,
		hash: snapshot.hash
	};
	return {
...
//#endregion
export { restoreRedactedValues as a, mapSensitivePaths as c, redactConfigSnapshot as i, applyDerivedTags as l, redactConfigObject as n, applySensitiveHints as o, redactConfigRaw as r, buildBaseHints as s, REDACTED_SENTINEL as t };