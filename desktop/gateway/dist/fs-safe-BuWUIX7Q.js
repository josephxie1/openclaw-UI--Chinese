...
import { d as hasNodeErrorCode, f as isNotFoundPathError, m as isSymlinkOpenError, p as isPathInside, s as sameFileIdentity } from "./openclaw-root-3Za7U6qY.js";
...
async function writeFileWithinRoot(params) {
	...
	try {
		try {
			handle = await fsPromises.open(ioPath, OPEN_WRITE_EXISTING_FLAGS, 384);
		} catch (err) {
			if (!isNotFoundPathError(err)) throw err;
			handle = await fsPromises.open(ioPath, OPEN_WRITE_CREATE_FLAGS, 384);
	...
	try {
		const [stat, lstat] = await Promise.all([handle.stat(), fsPromises.lstat(ioPath)]);
		...
		const realPath = await fsPromises.realpath(ioPath);
		...
		if (createdForWrite && err instanceof SafeOpenError && openedRealPath) await fsPromises.rm(openedRealPath, { force: true }).catch(() => {});
		throw err;
	} finally {
		await handle.close().catch(() => {});
	}
