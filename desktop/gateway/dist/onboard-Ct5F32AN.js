...
import { t as applyOnboardingLocalWorkspaceConfig } from "./onboard-config-CnevR2ww.js";
...
async function runNonInteractiveOnboardingLocal(params) {
	...
	if (authChoice !== "skip") {
		const { applyNonInteractiveAuthChoice } = await import("./auth-choice-6pNM-Kx-.js");
	...
	if (opts.installDaemon) {
		const { installGatewayDaemonNonInteractive } = await import("./daemon-install-Ccsyhq5v.js");
	...
	if (!opts.skipHealth) {
		const { healthCommand } = await import("./health-DDpN1HPa.js");
