...
import { D as danger, F as success, O as info, x as shortenHomePath, z as theme } from "./utils-XeuG5BG2.js";
...
import { _ as isBlockedObjectKey } from "./session-key-Cuihcz_O.js";
...
import { t as redactConfigObject } from "./redact-snapshot-BpYHj_2r.js";
...
function formatConfigIssueLines(issues, marker) {
...
async function loadValidConfig(runtime = defaultRuntime) {
	...
	for (const line of formatConfigIssueLines(snapshot.issues, "-")) runtime.error(line);
	...
	try {
		...
		if (!snapshot.valid) {
			...
			if (opts.json) runtime.log(JSON.stringify({
				...
				for (const line of formatConfigIssueLines(issues, danger("×"))) runtime.error(`  ${line}`);
				runtime.error("");
...
function registerConfigCli(program) {
	const cmd = program.command("config").description("Non-interactive config helpers (get/set/unset/file/validate). Run without subcommand for the setup wizard.").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.openclaw.ai/cli/config")}\n`).option("--section <section>", "Configure wizard sections (repeatable). Use with no subcommand.", (value, previous) => [...previous, value], []).action(async (opts) => {
		const { configureCommandFromSectionsArg } = await import("./configure-D7PP2JD9.js").then((n) => n.t);
